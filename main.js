...
//console.log(keyword);
 function getKeywordFormUrl(sParam) {
...
$( document ).ready(function()
{
	...
	if ($("iframe").hasClass("livestream_espot"))
		{
	...
	$('.poster').each(function() {
		...
		var newUrl =(chageSRCtofit(imageDataSource))
	  var setImageSource = $(this).attr('src', newUrl);
	});
	//chage img src
	
	//send search form
$(".search_staff").on("submit",function (event) {
...
$(".kidturn").on("submit",function (event) {
	event.preventDefault();
	var amargi='';var hmargin='';var sumG=0;
	  var fd = new FormData();
		 var etcData = $(this).serializeArray();
	    $.each(etcData,function(key,input){
        fd.append(input.name,input.value);
			if(input.name=='turnover')
			{ 
				 turnover=input.value;
			}	
			if(input.name=='gcost')
			{
				 gcost=input.value;
		...
		// console.log(turnover+'*'+sumG+'*'+gcost+'*'+margin);
		
while(sumG<turnover)
{
		 sumG=sumG+Number(gcost);
		  gcost=(Number(gcost)* ((1/((100+(Number(margin)))/2)*100)-1) );
if(gcost<1)break;
...
if(gcost<1){
		$('.margin_cal_result').html('<div class="success_result_text">*คุณเสียเงินเดิมพันทั้งหมด</div>');	
}else{
	$('.margin_cal_result').html('<div class="success_result_text">เมื่อคุณเล่นจบครบเทิร์นคุณเหลือเงิน <span class="green_t">'+(Number(gcost).toFixed(2))+'</span></div>');	
}
});



$(".margin_cal").on("submit",function (event) {
	event.preventDefault();
	var amargi='';var hmargin='';
	  var fd = new FormData();
		 var etcData = $(this).serializeArray();
	    $.each(etcData,function(key,input){
        fd.append(input.name,input.value);
	
		if(input.name=='hmargin')
		{
			 
			 hmargin=input.value;
		}	
		if(input.name=='amargin')
		{
			 amargin=input.value;
		}	

    });
		if(hmargin>0&&amargin>0) 
		{
				var gcost=Number(100*(1-((1/hmargin)+(1/amargin)))).toFixed(2);
				var payout=Number(100+Number(gcost)).toFixed(2);
				
				if(payout>100)
				{
					text_payout='<span class="green_t">'+payout+'%</span>';
				}else
				{
					text_payout='<span >'+payout+'%</span>';
				}
				
				if(gcost<0)
				{
					text_g='คุณเสียค่าน้ำคู่นี้ <span class="red_t">'+gcost+'%</span>';
				}else
				{
					text_g='คุณได้ค่าน้ำคู่นี้ <span class="green_t">'+gcost+'%</span>';
				}
			$('.margin_cal_result').html('<div class="success_result_text">เจ้ามือจ่ายคู่นี้ '+text_payout+' '+text_g+' </div>');	
		}else
		{
		$('.margin_cal_result').html('<div class="error_result_text">คุณต้องกรอกข้อมูลให้ถูกต้อง ครบถ้วน</div>');	
		}
//$gcost=100*(1-((1/$hmargin)+(1/$amargin)));
});
